...
foreach(mini_dice_options() as $chance=>$mult){$dev=$chance/100*$mult;printf("Dice %2d%%: multiplier %.2f EV %.4f\n",$chance,$mult,$dev);if($dev<.94||$dev>.98)throw new RuntimeException('Dice EV outside engineering bounds');}
for($level=1;$level<=8;$level++){$prob=pow(2/3,$level);$mult=mini_tower_multiplier($level);$tev=$prob*$mult;printf("Tower floor %d: survival %.5f multiplier %.2f EV %.4f\n",$level,$prob,$mult,$tev);if($tev<.94||$tev>.98)throw new RuntimeException('Tower EV outside engineering bounds');}
