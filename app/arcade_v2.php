...
function arcade_game_spin_hv(PDO $pdo,int $userId,string $gameKey,int $betRub): array {
    ...
    try{
        ...
        $summary=['free'=>$isFree,'free_spins_after'=>$free,'feature'=>!empty($result['payload']['feature']),'math_profile'=>'high-volatility-v2'];
        ...
        $pdo->commit();
        return array_merge(['ok'=>true,'round_id'=>$round,'game'=>$gameKey,'bet'=>$betRub,'is_free_spin'=>$isFree,'cost'=>$cost/100,'balance_before'=>$before/100,'balance_after'=>$after/100,'total_win'=>$win/100,'free_spins'=>$free,'math_profile'=>'high-volatility-v2'],$result['payload']);
