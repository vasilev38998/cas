...
/** Грозовая кузница: scatter-pays 8+, каскады и растущий множитель ковки. */
function hv_forge_tempest(int $betK,bool $isFree,int &$free,array &$state): array {
    ...
    $weights=['ember'=>1880,'hammer'=>1760,'rune'=>1620,'shield'=>1460,'crown'=>1180,'thunder'=>960,'wild'=>$isFree?135:72,'scatter'=>54];
    $base=['ember'=>1.0,'hammer'=>1.08,'rune'=>1.18,'shield'=>1.35,'crown'=>1.65,'thunder'=>2.05];
    $tiers=[8=>.24,10=>.42,12=>.78,15=>1.45,18=>2.6,22=>5.0,26=>9.0];
    $grid=ag_grid($c,$r,$weights);$initial=$grid;$steps=[];$total=0;$mult=$isFree?max(1,(int)($state['forge_mult']??1)):1;
    for($cascade=1;$cascade<=12;$cascade++){
    ...
    return ['win'=>$total,'payload'=>['initial_grid'=>$initial,'steps'=>$steps,'feature'=>null,'free_spins_awarded'=>$award,'badge'=>$isFree?'ГРОЗОВАЯ КОВКА ×'.max(1,(int)($state['forge_mult']??1)):'8+ В ЛЮБОМ МЕСТЕ • КАСКАДЫ','forge_multiplier'=>$mult]];
}

/** Лунный зверинец: линии и случайные полные wild-колонны. */
function hv_lunar_beasts(int $betK,bool $isFree,int &$free,array &$state): array {
    $c=5;$r=4;$weights=['fox'=>2050,'owl'=>1900,'wolf'=>1660,'stag'=>1390,'lynx'=>1120,'wild'=>90,'scatter'=>64];
    $pay=['fox'=>[3=>1.15,4=>3.5,5=>10.5],'owl'=>[3=>1.35,4=>4.2,5=>13],'wolf'=>[3=>1.7,4=>5.6,5=>18],'stag'=>[3=>2.4,4=>8.5,5=>28],'lynx'=>[3=>3.8,4=>14,5=>46],'wild'=>[3=>6.5,4=>24,5=>78]];
    $grid=ag_grid($c,$r,$weights);$initial=$grid;$phase=$isFree?max(1,(int)($state['moon_phase']??1)):1;$moonCols=[];
    $chance=$isFree?min(52,20+$phase*5):7;$roll=random_int(1,100);
    if($roll<=$chance){$cols=range(0,$c-1);shuffle($cols);$count=($isFree&&$roll<=max(5,(int)floor($chance*.22)))?2:1;$moonCols=array_slice($cols,0,$count);foreach($moonCols as $col)for($row=0;$row<$r;$row++)$grid[$col*$r+$row]='wild';}
...
function exp_clockwork_factor(int $n): float {return $n>=22?5.5:($n>=18?3.0:($n>=14?1.45:($n>=11?.82:($n>=8?.42:.22))));}

/** Механический импульс: кластеры и шестерни, очищающие крестом строку/колонну. */
function hv_clockwork_shift(int $betK,bool $isFree,int &$free,array &$state): array {
    ...
    for($cascade=1;$cascade<=11;$cascade++){
        ...
        $cross=[];$gearBonus=0;foreach(array_keys($triggered) as $g){$col=intdiv($g,$r);$row=$g%$r;for($rr=0;$rr<$r;$rr++)$cross[]=$col*$r+$rr;for($cc=0;$cc<$c;$cc++)$cross[]=$cc*$r+$row;$gearBonus+=(int)round($betK*.22*$gearPower);if($isFree)$gearPower=min(6,$gearPower+1);}
