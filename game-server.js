(() => {
  ...
  S.fillPaytable=()=>{S.$('#paytableList').innerHTML=S.PAY_KEYS.map(k=>{const x=S.SYMBOLS[k],p=x.pay;return `<div class="pay-row"><div class="pay-icon">${S.svgFor(k)}</div><div><strong>${x.name}</strong><small>6: ${(p[6]*S.PAYOUT_SCALE).toFixed(2)}× • 9: ${(p[9]*S.PAYOUT_SCALE).toFixed(2)}× • 12: ${(p[12]*S.PAYOUT_SCALE).toFixed(2)}× • 15+: ${(p[15]*S.PAYOUT_SCALE).toFixed(2)}× ставки</small></div></div>`;}).join('')+`<div class="pay-row"><div class="pay-icon">${S.svgFor('scatter')}</div><div><strong>Бонусный автомат</strong><small>3 = 8 • 4 = 10 • 5+ = 12 фриспинов</small></div></div>`;};
